/* GALERIA DE CASOS
   ...
   Foto clinica em material de divulgacao nao e livre. Antes de publicar
   qualquer imagem real nesta secao, confirmar com o CRO-RS:
   ...
   3. Sem identificacao do paciente e sem promessa de resultado, explicita
      ou sugerida pela composicao das imagens.
   4. Numero de CRO visivel no material.

   Nao publicar esta secao com foto real sem esse check.
   ========================================================================== */
...
const casos = [
  { id: "caso-01", ratio: "3 / 2", label: "[ caso 01 ]", span: 7 },
  { id: "caso-02", ratio: "4 / 5", label: "[ caso 02 ]", span: 5, video: "/video/resinart-caso.mp4" },
  { id: "caso-03", ratio: "4 / 5", label: "[ caso 03 ]", span: 4 },
  { id: "caso-04", ratio: "3 / 2", label: "[ caso 04 ]", span: 8 },
            ...
            <figure key={c.id} className="gallery-item" data-span={c.span}>
              <ParallaxMedia ratio={c.ratio}>
                {"video" in c && c.video ? (
                  <VideoTile
                    src={c.video}
                    label="Vídeo do procedimento. Conteúdo ainda não revisado para conformidade CRO."
                  />
                ) : (
                <>
                {/* Quando a foto real chegar, trocar por:
                    <MediaSlot ratio={c.ratio} rotulo={c.label}
                      src="/casos/caso-01.jpg" width={1800} height={1200}
                      alt="descrição clínica objetiva do que a imagem mostra" />
                    O alt e obrigatorio e nao pode ser "foto de caso":
